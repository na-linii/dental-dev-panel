...
def seed_dataset(langfuse):
    """Create or update dataset with edge case items."""
    try:
        langfuse.get_dataset(DATASET_NAME)
        print(f"  Dataset '{DATASET_NAME}' exists, skipping seed")
        return
    except Exception:
        pass
    ...
    langfuse.create_dataset(
        ...
        description="Edge case test scenarios for dental clinic AI agent. "
                    "Covers booking, FAQ, security, confirmation, and production bugs.",
        metadata={"version": "1.0", "source": "docs/edge-cases.md"},
    )
...
def main():
    ...
    print(f"  Evaluators: security, handoff, dialog")
    ...
    result = dataset.run_experiment(
        ...
        task=call_agent,
        evaluators=[security_evaluator, handoff_evaluator, dialog_evaluator],
    )
